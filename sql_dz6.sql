...
DELIMITER //
CREATE PROCEDURE move_user_to_old(IN user_id BIGINT UNSIGNED)
BEGIN
    ...
    INSERT INTO users_old SELECT * FROM users WHERE id = user_id; -- copy user to users_old
    DELETE FROM users WHERE id = user_id; -- delete user from users
    COMMIT; -- commit transaction
    SELECT "User moved to users_old successfully.";
END //
DELIMITER ;
...
) ROW_FORMAT=COMPRESSED ENGINE=ARCHIVE;

CREATE TRIGGER user_insert_trigger AFTER INSERT ON users FOR EACH ROW
INSERT INTO logs (table_name, primary_key_id) VALUES ('users', NEW.id);

CREATE TRIGGER community_insert_trigger AFTER INSERT ON communities FOR EACH ROW
INSERT INTO logs (table_name, primary_key_id) VALUES ('communities', NEW.id);

CREATE TRIGGER message_insert_trigger AFTER INSERT ON messages FOR EACH ROW
